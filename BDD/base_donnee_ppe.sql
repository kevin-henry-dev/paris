...
(null,'23 rue Louis delacarte', 'concert', '1996-12-12', 10),
(null,'14 rue Dunoyer','tournoi de tennis', '1997-11-11', 20 );
insert into exterieur values
(null, '10 rue Etienne Dolet', 'cinema', '1998-10-10', 'bonne');
...
for each row 
begin
	insert into archivePrivee values (old.idC, old.ville, old.codePostal, old.prix, old.bourse);
end //
...
for each row 
begin
	insert into archivePublique values (old.idC, old.ville, old.codePostal, old.prix, old.reduction);
end //
...
for each row 
begin
	insert into archivePersonne values (old.idP, old.pseudo, old.mdp, old.nom, old.prenom, old.adresse, old.tel, old.cp, old.email, old.datenaiss, old.sexe, old.fonction);
end //
...
drop trigger if exists mariageBeforeUpdate
delimiter //

create trigger mariageBeforeUpdate
before update on mariage
for each row
begin
	if new.datedivorce is not null
	then
		delete from mariage where idP1 = new.idP1;
	end if;
end //
delimiter;
*/
...
drop trigger if exists mariageBeforeInsert;
delimiter //

create trigger mariageBeforeInsert
before insert on mariage
for each row 
begin

if new.idP1 in (select idP1 from mariage) or new.idP1 in (select idP2 from mariage)
then
	if (select ifnull(datedivorce, 0) from mariage where (idP1 = new.idP1 or idP2 = new.idP1) and dateMariage>=all(select dateMariage from mariage where idP1 = new.idP1 or idP2 = new.idP1)) = 0	
	then
		signal sqlstate '45000'
		set message_text='personne deja mariee';
	end if;
end if ;


if new.idP2 in (select idP2 from mariage) or new.idP2 in (select idP1 from mariage)
then
	if (select ifnull(datedivorce, 0) from mariage where (idP2 = new.idP2 or idP1 = new.idP2) and dateMariage>=all(select dateMariage from mariage where idP2 = new.idP2 or idP1 = new.idP2)) = 0	
	then
		signal sqlstate '45000'
		set message_text='personne deja mariee';
	end if;
